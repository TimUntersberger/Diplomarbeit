...
Bis jetzt musste man den Computer physisch mit einem Kabel mit dem Mikrocontroller verbinden um neue Firmware auf den Controller zu spielen.
...
\begin{figure}[H]
    \begin{center}
        \includegraphics[scale=0.55]{diagrams/ota_partition_table.png}
...
\begin{figure}[H] \begin{center}
    \includegraphics[scale=0.4]{diagrams/mesh-node-types.png}
...
Cytoscape ist eine in Javascript geschriebene Open-Source-Library für Graphentheorie.
...
In der \textbf{Dht22} Library \cite{dht22-lib} sind insgesamt sechs öffentliche Funktionen definiert, wie in der folgenden Abbildung zu sehen ist:
...
Die HTTP Library definiert und implementiert drei verschiedene Funktionen, welche in  Abbildung \ref{abb:http_functions} zu sehen sind.
...
Die \textbf{mosquitto.conf} Datei konfiguriert den Mosquitto Broker so, dass dieser auf zwei Ports Daten erwartet.
...
\begin{figure}[H]
    \begin{center}
        \includegraphics[scale=0.5]{images/example_mesh_visualizer_website.png}
...
Dieser beinhaltet nicht nur den Source Code sondern auch die \textit{menuconfig} Datei, welche in Kapitel \ref{sec:esp-idf-toolchain} genauer beschrieben ist.
