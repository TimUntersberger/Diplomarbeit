...
% TODO: Change Diagram 
%   digital ocean droplet -> host machine
%   Mesh -> tcp
%   http -> mqtt
...
\begin{itemize}
    \item Ein \textbf{Mikrocontroller} welcher mit dem DHT22 verbunden ist und regelmäßig die Sensordaten weiterschickt 
    \item Ein \textbf{Root Mikrocontroller} der nur für das weiterleiten der Nachrichten zuständig ist.
\end{itemize}

Der Root Mikrocontroller ist mit dem MQTT Broker, welcher in diesem Fall ein \textbf{Mosquitto} Broker ist, verbunden.

Der \textbf{Mosquitto} Server ist auf einer beliebigen \textbf{Host Machine} (Bsp.: Digital Ocean Droplet) gestartet.
...
Das obige Diagramm zeigt den Verlauf von einer Iteration dieses Beispieles an.
Der \textbf{dhtmc} (Der \textbf{Mikrocontroller} des Deployment Diagramms) ließt die aktuellen Werte des DHT22, welche er dann in das Mesh Netzwerk verschickt.
...
Die folgenden Anweisungen sind für Ubuntu gedacht. Sie können für Alternativen wie zum Beispiel das \textbf{Windows Subsystem for Linux (WSL) \ref{sec:wsl}} funktionieren, sie müssen aber nicht. 
...
Damit man die ESP-IDF Libraries benutzen kann, muss man zuerst diese herunterladen.
...
Mit den obigen Befehlen erstellt man zuerst einen neuen Ordner im Homeverzeichnis von dem aktuell eingeloggten Benutzer und wechselt dann in diesen. 
Anschließend wird das offizielle Repository von ESP-IDF rekursiv gecloned.

Nachdem das Repository erfolgreich gecloned wurde, muss man die Libraries installieren. Dies kann man durch folgende Bash Befehle lösen.
...
Diese Befehle ändern das aktuelle Verzeichnis auf das vorher installierte Repository und führt dann die \textbf{install.sh} Datei von ESP-IDF aus.
...
Danach müssen noch ein paar Umgebungsvariable von ESP-IDF selber gesetzt werden, dafür muss man den obigen Befehl ausführen.

Diese Variablen werden nur für die aktuelle Shell-Session gesetzt, deswegen wäre es eine gute Entscheidung diese Befehle in die \textbf{.bashrc} Datei im Homeverzeichnis zu schreiben.
...
Anschließend muss der Terminal neugestartet werden um die \textbf{.bashrc} Datei benutzen zu können.

\subsubsection{WSL}\label{sec:wsl}

WSL unterstützt laut dem Stand vom \textbf{29.03.2020} das Linux USB Interface nicht. Dies bedeutet, dass das ESP-IDF Setup zwar funktionieren kann, aber die Benutzung der Toolchain nicht realistisch ist.
...
% Describe the docker-compose file above

\subsection{Mesh Visualizer}
\subsection{Config}
\subsection{Code}
