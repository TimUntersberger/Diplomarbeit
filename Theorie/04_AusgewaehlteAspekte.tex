...
\textmd{ESP-MESH} ist ein Netzwerkprotokoll, das auf dem Wi-Fi-Protokoll basiert. Mit \textmd{ESP-MESH} können zahlreiche Geräte (im Folgenden als Knoten bezeichnet), die über einen großen physischen Bereich (sowohl drinnen als auch draußen) verteilt sind, unter einem einzigen WLAN (Wireless Local Area Network) miteinander verbunden werden. \textmd{ESP-MESH} ist selbstorganisierend und selbstheilend, was bedeutet, dass das Netzwerk autonom aufgebaut und gepflegt werden kann.
...
\textmd{ESP-MESH} unterscheidet sich von herkömmlichen Wi-Fi-Netzwerken darin, dass Knoten keine Verbindung zu einem zentralen Knoten herstellen müssen. Stattdessen dürfen Knoten eine Verbindung zu benachbarten Knoten herstellen.

Knoten sind gegenseitig für die Weiterleitung der Übertragungen verantwortlich. Dies ermöglicht es einem \textmd{ESP-MESH}-Netzwerk, einen viel größeren Versorgungsbereich zu haben, da Knoten immer noch mit verbunden sein können, ohne sich in Reichweite des zentralen Knotens befinden zu müssen. Ebenso ist \textmd{ESP-MESH} weniger anfällig für Überlastung, da die Anzahl der im Netzwerk zulässigen Knoten nicht mehr durch einen einzelnen zentralen Knoten begrenzt ist.

Um mit einem \textmd{ESP-MESH} Netzwerk umgehen zu können ist es notwendig folgende Begriffe zu verstehen:
\begin{itemize}
    \item \textmd{Node}
    
    Jedes Gerät, das Teil eines ESP-MESH-Netzwerks ist
    \item \textmd{Root Node}
    
    Der \textmd{Root Node} ist der oberste Knoten eines Netzwerk

    \item \textmd{Child Node}
    
    Ein \textmd{Child Node} ist ein Knoten, der sich auf der hierarchischen Ebene mindestens eine Stuffe \textmd{unter} einem anderem Knoten befindet

    \item \textmd{Parent Node}
    
    Ein \textmd{Parent Node} ist ein Knoten, der sich auf der hierarchischen Ebene mindestens eine Stuffe \textmd{über} einem anderem Knoten befindet.

    \item \textmd{Descendant Node}
    
    Ein \textmd{descendant Node} ist jeder Knoten, der durch wiederholtes Weitergehen von Eltern zu Kind erreichbar ist
...
Cytoscape.js ist ein Open-Source-Projekt, und jeder kann einen Beitrag leisten. Weitere Informationen finden Sie in der GitHub README.
...
\begin{figure}[H]
    \begin{center}
        ...
        \caption{HTTP Client Methoden}%\cite{esp_idf_http_client_methods}}
...
% TODO: Juli help split sentence yes
