 ...
 */
void EspWifiManagerClass::startWifi()
{
    isConnecting = true;
    ...
    memcpy(stationConfig.ssid, ssid, strlen(ssid));
    memcpy(stationConfig.password, password, (strlen(password) + 1));
    wifi_config_t wifi_config = {};
...
bool EspWifiManagerClass::getIsConnecting()
{
    return isConnecting;
}

bool EspWifiManagerClass::getIsConnected()
{
    return isConnected;
}
